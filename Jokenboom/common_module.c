...
int configura_addr(const char *str_addr, const char *porta_str, struct sockaddr_storage *server_addr){
    ...
    uint16_t port = (uint16_t)atoi(porta_str); // Converte a string da porta para um número intero
    port = htons(port); // Converte o número inteiro da porta para big-endian (usado em rede)

    struct in_addr endereco_v4; 
    struct in6_addr endereco_v6; 

    if(inet_pton(AF_INET, str_addr, &endereco_v4)){ // inet_pton converte str_addr para binário 
        struct sockaddr_in *addr_v4 = (struct sockaddr_in *)server_addr;
        addr_v4 -> sin_family = AF_INET; // Definição da família
        addr_v4 -> sin_port = port; // sin_port armazena a porta em formato big-endian
...
int inicializar_addr_server(const char *protocolo, const char* porta_str, struct sockaddr_storage *server_addr){
    ...
    uint16_t port = (uint16_t)atoi(porta_str); 
    port = htons(port); 
    ...
    if(strcmp(protocolo, "v4") == 0){
        ...
        addr_v4 -> sin_port = port; // Definição da porta do servidor 
        return 0;    
    } 
